...
// Start from ~Mar 2026 (slot 405M) — captures recent weeks of jupSOL activity
...
// Two separate queries — mints (a0 = jupSOL mint) and burns (a1 = jupSOL mint)
    ...
    .pipe((blocks: any[]) => {
      ...
      for (const block of blocks) {
        ...
        for (const ins of block.instructions) {
          ...
          // Verify it's jupSOL mint
          if (mintAccount !== JUPSOL_MINT) continue
    ...
    .pipeTo(
      clickhouseTarget({
        ...
        onData: async ({ data, store }) => {
          const ops = data.ops as JupsolOp[]
