...
async function main() {
  ...
  const [{ cnt }] = await q<{ cnt: string }>('SELECT count() as cnt FROM jupsol_ops FINAL')
  const rowCount = Number(cnt)
  if (rowCount > 0) pass(`Row count: ${rowCount.toLocaleString()} jupSOL operations`)
  else { fail('Row count is 0'); process.exit(1) }

  const cols = await q<{ name: string }>("SELECT name FROM system.columns WHERE database = currentDatabase() AND table = 'jupsol_ops'")
  ...
  const [{ t0, t1 }] = await q<{ t0: string; t1: string }>('SELECT min(timestamp) as t0, max(timestamp) as t1 FROM jupsol_ops FINAL')
  pass(`Timestamp range: ${t0.slice(0, 10)} to ${t1.slice(0, 10)}`)

  const actions = await q<{ a: string; n: string }>('SELECT action as a, count() as n FROM jupsol_ops FINAL GROUP BY a ORDER BY n DESC')
  pass(`Actions: ${actions.map(e => `${e.a}=${Number(e.n).toLocaleString()}`).join(', ')}`)

  const [{ empty }] = await q<{ empty: string }>("SELECT countIf(tx_signature = '') as empty FROM jupsol_ops FINAL")
  if (Number(empty) === 0) pass('No empty tx signatures')
  else fail(`${empty} empty tx signatures`)

  const [{ neg }] = await q<{ neg: string }>('SELECT countIf(amount_lamports = 0) as neg FROM jupsol_ops FINAL')
  ...
  const [{ midSlot }] = await q<{ midSlot: string }>('SELECT toUInt64(min(slot) + (max(slot) - min(slot)) * 0.5) as midSlot FROM jupsol_ops FINAL')
  const fromSlot = Number(midSlot)
  const toSlot = fromSlot + 500

  // Query Portal for MintTo instructions with a0 = jupSOL mint
  const portalBody = JSON.stringify({
    ...
    instructions: [
      { programId: [TOKEN_PROGRAM], d1: ['0x07', '0x0e'], a0: [JUPSOL_MINT] },
      { programId: [TOKEN_PROGRAM], d1: ['0x08', '0x0f'], a1: [JUPSOL_MINT] },
    ],
  })
  ...
  try {
    ...
    const [{ chCnt }] = await q<{ chCnt: string }>(`SELECT count() as chCnt FROM jupsol_ops FINAL WHERE slot >= ${fromSlot} AND slot <= ${toSlot}`)
  ...
  const spotChecks = await q<{ tx_signature: string; slot: string; action: string; amount_sol: string }>(
    "SELECT tx_signature, slot, action, amount_sol FROM jupsol_ops FINAL WHERE amount_sol > 0 ORDER BY slot LIMIT 3"
  )

  for (const sc of spotChecks) {
    const isValid = sc.tx_signature.length >= 64 && Number(sc.amount_sol) > 0
    if (isValid) {
      pass(`Spot-check ${sc.tx_signature.slice(0, 16)}... slot ${sc.slot}: ${sc.action} ${Number(sc.amount_sol).toFixed(2)} jupSOL`)
    } else {
