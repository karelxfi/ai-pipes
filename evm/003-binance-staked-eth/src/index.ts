...
const custom = evmDecoder({
  range: { from: '17200000' },
  contracts: ['0xa2E3356610840701BDf5611a53974510Ae27E2e1'],
  /**
   ...
   */
  events: {
    ExchangeRateUpdated: wbethEvents.ExchangeRateUpdated,
  },
}).pipe(enrichEvents)
    ...
    .pipeTo(
      clickhouseTarget({
        ...
        onData: async ({ data, store }) => {
          await store.insert({
            table: 'wbeth_exchange_rate_updated',
            values: toSnakeKeysArray(data.custom.ExchangeRateUpdated),
            format: 'JSONEachRow',
          })
        },
        onRollback: async ({ safeCursor, store }) => {
          await store.removeAllRows({
            tables: ['wbeth_exchange_rate_updated'],
