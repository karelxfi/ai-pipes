...
const client = createClient({
  ...
  database: process.env.CLICKHOUSE_DATABASE ?? 'benqi_lending',
  username: process.env.CLICKHOUSE_USER ?? 'default',
  password: process.env.CLICKHOUSE_PASSWORD ?? 'password',
})

const QI_USDC_N = '0xb715808a78f6041e46d61cb123c9b4a27056ae9c'
...
async function main() {
  ...
  if (rowCount > 0) pass(`${rowCount} rows in lending_events`)
  else fail('No rows')
  ...
  if (types.length >= 4) pass(`${types.length} event types: ${types.map((r: any) => r.event_type).join(', ')}`)
  else fail(`Only ${types.length} event types`)

  const markets = await q("SELECT distinct market FROM lending_events")
  if (markets.length >= 5) pass(`${markets.length} markets: ${markets.map((r: any) => r.market).join(', ')}`)
  else fail(`Only ${markets.length} markets`)

  console.log('\n=== Phase 2: Portal Cross-Reference ===')

  const [{ minBlock, maxBlock }] = await q("SELECT min(block_number) minBlock, max(block_number) maxBlock FROM lending_events WHERE market = 'qiUSDCn'")
  const midBlock = Math.floor((Number(minBlock) + Number(maxBlock)) / 2)
  const fromBlock = midBlock
  const toBlock = midBlock + 2000

  const [{ chCount }] = await q(`SELECT count() chCount FROM lending_events WHERE market = 'qiUSDCn' AND block_number >= ${fromBlock} AND block_number < ${toBlock}`)

  try {
    const resp = await fetch('https://portal.sqd.dev/datasets/avalanche-mainnet/stream', {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        fromBlock,
        toBlock,
        logs: [{ address: [QI_USDC_N], topic0: ALL_TOPICS }],
  ...
  for (const tx of txRows) {
    try {
      ...
      if (resp.ok) {
        ...
        for (const line of text.split('\n')) {
          if (!line.trim()) continue
          try { if ((JSON.parse(line).logs ?? []).length > 0) found = true } catch {}
        }
        if (found) pass(`Spot-check tx ${tx.tx_hash.substring(0, 10)}... block=${blockNum}, ${tx.market} ${tx.event_type} — Portal confirms`)
