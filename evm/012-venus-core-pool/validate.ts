...
const client = createClient({
  ...
  database: process.env.CLICKHOUSE_DATABASE ?? 'venus',
...
async function main() {
  console.log('=== Venus Core Pool BSC Lending — Validation ===\n')

  console.log('── Phase 1: Structural Checks ──')
  const [{ count }] = await query<{ count: string }>('SELECT count() as count FROM venus_actions')
  const rowCount = Number(count)
  if (rowCount > 0) pass(`Row count: ${rowCount}`)
  else { fail('Row count is 0'); process.exit(1) }

  const cols = await query<{ name: string }>(`SELECT name FROM system.columns WHERE database = '${process.env.CLICKHOUSE_DATABASE ?? 'venus'}' AND table = 'venus_actions'`)
  const required = ['market', 'market_name', 'action', 'user', 'block_number', 'tx_hash', 'tx_index', 'log_index', 'timestamp']
  ...
  const actions = await query<{ action: string; cnt: string }>('SELECT action, count() as cnt FROM venus_actions GROUP BY action ORDER BY cnt DESC')
  ...
  const markets = await query<{ market_name: string; cnt: string }>('SELECT market_name, count() as cnt FROM venus_actions GROUP BY market_name ORDER BY cnt DESC')
  markets.forEach(m => console.log(`  ${m.market_name}: ${m.cnt} events`))
  if (markets.length >= 2) pass(`${markets.length} markets indexed`)
  else fail('Expected at least 2 markets')

  const [{ min_ts, max_ts }] = await query<{ min_ts: string; max_ts: string }>('SELECT min(timestamp) as min_ts, max(timestamp) as max_ts FROM venus_actions')
  pass(`Timestamp range: ${min_ts} to ${max_ts}`)

  // Phase 2: Portal cross-reference (BSC!)
  console.log('\n── Phase 2: Portal Cross-Reference ──')
  const [{ min_block, max_block }] = await query<{ min_block: string; max_block: string }>('SELECT min(block_number) as min_block, max(block_number) as max_block FROM venus_actions')
  const midBlock = Math.floor((Number(min_block) + Number(max_block)) / 2)
  const sampleFrom = midBlock, sampleTo = midBlock + 10000

  const [{ ch_count }] = await query<{ ch_count: string }>(`SELECT count() as ch_count FROM venus_actions WHERE block_number >= ${sampleFrom} AND block_number <= ${sampleTo}`)
  ...
  // NOTE: BSC uses binance-mainnet dataset
  const portalUrl = 'https://portal.sqd.dev/datasets/binance-mainnet/stream'
  ...
  const samples = await query<{ block_number: string; tx_hash: string; action: string; market: string; market_name: string }>(
    'SELECT block_number, tx_hash, action, market, market_name FROM venus_actions ORDER BY block_number DESC LIMIT 3'
  )
