...
export interface EnrichedEventMeta { blockNumber: number; txHash: string; logIndex: number; timestamp: number; contract: string }
export function enrichEvents<T extends EventResponse<Events, string[]>>(obj: T): ExtendEventResponse<T, EnrichedEventMeta> {
  ...
  for (const key in obj) {
    ...
    result[key] = (value as any[]).map((v) => ({
      ...v.event, blockNumber: v.block.number, txHash: v.rawEvent.transactionHash,
      logIndex: v.rawEvent.logIndex, timestamp: new Date(v.timestamp).getTime() / 1000, contract: v.contract,
